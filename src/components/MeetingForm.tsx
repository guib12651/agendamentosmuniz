    ...
    try {
      ...
      if (editMeeting) {
        ...
        const successInfo = {
          ...
          preSeller: form.preSeller,
          consultant: form.consultant,
        };
        const { id, userId: _uid, ...rest } = form as any;
        console.log("Adding meeting with data:", { ...rest, time: form.time });
        await addMeeting({ ...rest, time: form.time }, userId);
        setForm({ ...emptyForm, preSeller: userDisplayName } as any);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          <Label>Usuário *</Label>
          <Input value={form.preSeller} onChange={(e) => set("preSeller", e.target.value)} placeholder="Nome do usuário" className="h-12 sm:h-10 text-base sm:text-sm" readOnly={!isAdmin} />
        </div>
        <div className="space-y-1.5">
